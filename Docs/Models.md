...
- `CommandBoundary` reduces a strictly sequenced batch into one atomic `PlaybackSnapshot`. Timeline restarts receive a new phase session while retaining `LogicalPlaybackID`; sound changes cannot publish until preparation and active-renderer adoption are confirmed, and stopped sound changes prepare only the next session.
