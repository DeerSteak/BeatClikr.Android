...
`AudioPlayerService` owns `MetronomeAudioEngine` behind `PlaybackEnginePort`. Engine timing delegates remain only below that port to prompt committed-event draining; `AudioPlayerService` owns this compatibility seam, which Phase 9 should remove if event draining becomes entirely scheduler-driven.
