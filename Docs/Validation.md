...
The current lifecycle contracts are distinct. Rotation and Activity recreation retain application-scoped playback authority and reconstruct visible UI/keep-awake state without a restart. Process background and screen-off cross the process `STOPPED` boundary and tear down foreground-only playback; this intentionally differs from deferred PL-007. Process death always recreates `Idle` and cannot restore authority from saved state. Secondary outputs independently follow process `STARTED`: Activity recreation and transient overlays do not suppress them, while process background or screen-off cancels pending effects and forces outputs off. The background-audio portion of this separation remains Phase 9 work.
