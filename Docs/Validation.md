...
The Phase 4.2 transport suite verifies immutable lifecycle transitions, same-session configuration amendments, prerequisite denial, tagged interruption and failure inputs, idempotent stop, and replacement without observable `Idle`. Engine start and stop acknowledgements, sound preparation publications, and renderer records carry stale guards. `Playing` requires prepared sounds plus opened route and backend evidence, while slow-tempo startup does not wait for rendering.
