...
`MetronomeAudioEngine` now selects the prepared frame session before the characterized legacy fallback. Handler callbacks continue to drive visual notifications but do not enqueue sounds while frame audio is active. Standard tempo, groove, and pattern changes enqueue an in-stream timeline replacement rather than stopping `AudioTrack`. The old timeline chooses its next event boundary; the replacement anchors there with the same absolute event index, preserving frame ownership, musical phase, monotonic event identity, and active waveform tails while the new tempo governs subsequent intervals.
...
Live polyrhythm changes use the same in-stream replacement mechanism. Because changing either ratio invalidates the individual slot grids, the old audio and visual timelines apply the pending configuration at their next coincident cycle boundary; the replacement starts both rhythms there and carries forward event and cycle identity. An active frame polyrhythm is retuned before any new-start fallback is considered, preventing a rejected frame start from reopening the legacy mixer alongside the old stream.
