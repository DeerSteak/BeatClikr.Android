...
The coordinator drains the bounded rendered-event ring while playback is active. Each committed event carries its intended frame and, when available, a frame-to-monotonic-time correlation; ViewModels and secondary outputs project that authoritative event without maintaining another musical timeline.
